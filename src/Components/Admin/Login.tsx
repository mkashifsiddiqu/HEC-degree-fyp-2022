...
const Login = () => {
  ...
  const onSubmit = async (e: MouseEvent) => {
   ...
   if (response.success) {
     const data={name:response.name,email:response.email,page:response.page}
     dispatch(loginuser(data))
     //Store  Cookies
     setCookie(`nameAdmin`,response.name) 
     setCookie(`emailAdmin`,response.email) 
     const page =  JSON.stringify(response.page)
     setCookie(`pageAdmin`,page)
    router.push(`/Admin/UniversitiesDetails`)
      setEmail(``)
       setPassword(``)
